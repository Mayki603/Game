...
const itemImages = [];
const itemData = [
    { src: 'assets/canned_food.png', points: 1, weight: 60, name: 'Консерва' },
    { src: 'assets/sushi.png', points: 2, weight: 25, name: 'Суші' },
    { src: 'assets/shrimp.png', points: 3, weight: 10, name: 'Креветка' },
    { src: 'assets/steak.png', points: 5, weight: 5, name: 'Стейк' }
];
...
function showStats() {
    ...
    totalDiv.innerHTML = `<h3>Усього очок: ${score}</h3><p>Час: ${formatTime(elapsedTime)}</p>`;
...
function resizeCanvas() {
    ...
    if (idleImage.height > 0) {
        ...
        if (!cat.isJumping) cat.y = canvas.height - 20 - cat.height;
    }
}
...
function update() {
    ...
    if (gameState === 'WAKING') {
        ...
        if (cat.frameTimer >= cat.wakeInterval) {
            ...
            else { gameState = 'LICKING'; cat.frameX = 0; }
            cat.frameTimer = 0;
        }
        return;
    }
    ...
    if (gameState === 'PLAYING') {
        ...
        if (cat.isJumping) {
            if (cat.isRunning) { nextImg = runJumpImage; totalFrames = cat.runJumpFrames; currentInterval = cat.runJumpInterval; }
            else { nextImg = jumpImage; totalFrames = cat.jumpFrames; currentInterval = cat.jumpInterval; }
        } else if (isMoving) {
        ...
        } else { nextImg = idleImage; totalFrames = cat.idleFrames; currentInterval = cat.idleInterval; }
        if (cat.currentImg !== nextImg) { cat.currentImg = nextImg; cat.frameX = 0; cat.frameTimer = 0; }
        cat.sourceWidth = cat.currentImg.width / totalFrames;
        cat.sourceHeight = cat.currentImg.height;
        cat.width = cat.sourceWidth * cat.scale;
        cat.height = cat.sourceHeight * cat.scale;
        ...
        if (cat.frameTimer >= currentInterval) {
            if (cat.isJumping) { if (cat.frameX < totalFrames - 1) cat.frameX++; }
            else { cat.frameX = (cat.frameX + 1) % totalFrames; }
            cat.frameTimer = 0;
        }
        updateItems();
    }
...
function draw() {
    ...
    ctx.save();
    if (cat.facingLeft) {
        ctx.drawImage(cat.currentImg, cat.frameX * cat.sourceWidth, 0, cat.sourceWidth, cat.sourceHeight, cat.x, cat.y, cat.width, cat.height);
    } else {
        ctx.translate(cat.x + cat.width / 2, cat.y + cat.height / 2);
        ctx.scale(-1, 1);
        ctx.drawImage(cat.currentImg, cat.frameX * cat.sourceWidth, 0, cat.sourceWidth, cat.sourceHeight, -cat.width / 2, -cat.height / 2, cat.width, cat.height);
    }
    ctx.restore();
    drawUI();
}
